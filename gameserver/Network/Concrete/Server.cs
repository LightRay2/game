...
namespace gameserver.Concrete
{
    class Server : IServer
    {
        ...
        //плюс закомментил раздельное управление клиентами 
        ...
        public void SendCommands(List<Command> commands)
        {
            ...
            commands.Sort((a,b)=>{
                int x = a.clientID.CompareTo(b.clientID);
                return x != 0 ? x : a.commandID.CompareTo(b.commandID);
            }
                ); //сортируем, чтобы все сделать в один проход
            int curClient=0;
            for(int i =0; i < commands.Count; i++)
            {
                ...
                catch (ArgumentOutOfRangeException) //todo тут тоже разные фигни могут случиться. Может быть он отвалился, пока мы ему команду создавали. Надо получше расписать
                {
                    ...
                    throw new Exception("клиента с id=" + commands[i].commandID.ToString() + " не существует. Проверьте, правильно ли вы задаете command.clientID");
                }
