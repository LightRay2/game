...
namespace Game2D.Game.Concrete
{
    class MenuMain
    {
        ...
        public void Process(ref Frame frame, IGetKeyboardState keyboard, DStateMain state)
        {
            ...
            frame.Add(new Sprite(ESprite.menuback, new Vector2(Config.ScreenWidth / 2, Config.ScreenHeight / 2, 0), Config.ScreenWidth, Config.ScreenHeight));

            Point2 curPos = POS;
            for (int i = 0; i < _active.items.Count; i++)
            {
                EFont font = i == selected ? SELECTED_FONT : MAIN_FONT;
                frame.Add(new Text(font, curPos, Config.LetterSize3, _active.items[i].text));
