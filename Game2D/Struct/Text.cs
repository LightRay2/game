    ...
    struct Text
    {
        ...
        public List<Sprite> GetSpritesWithRelativePos()
        {
            ...
            for(int i = 0; i < lines.Count; i++)
                ...
                {
                    if (ConfigOpengl.FontLetters.Contains(lines[i][j]))
                    {
                        Vector2 translation = new Vector2(0,0,-TextSize.x / 2 + j * letterWidth,
                            -TextSize.y / 2 + i * letterHeight);
                        translation.Rotate(pos.angleDeg);

                        Sprite toAdd = new Sprite(ESprite.end, new Vector2(pos.x + translation.vx, pos.y + translation.vy, pos.angleDeg), letterWidth, letterHeight, ConfigOpengl.FontLetters.IndexOf(lines[i][j]));
